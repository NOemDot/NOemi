...
class Player(wavelink.Player):
    ...
    async def add_tracks(self, ctx, tracks):
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            await ctx.send(f"Añadí **{tracks[0].title}** a la cola.")
        else:
            if (track := await self.choose_track(ctx, tracks)) is not None:
                self.queue.add(track)
                await ctx.send(f"Añadí {track.title} a la cola.")
    ...
    async def choose_track(self, ctx, tracks):
        ...
        embed.set_footer(text=f"Solicitados por:  {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def on_message(self, message):
      ...
      data[str(message.guild.id)]["music"] = data[str(message.guild.id)].get("music", {"channels": []})
      if len(data[str(message.guild.id)]["music"]["channels"]) != 0:
        if message.channel not in data[str(message.guild.id)]["music"]["channels"]:
          await message.channel.send("Canal no disponible.")
          return 
    ...
    async def cog_check(self, ctx):
        if isinstance(ctx.channel, discord.DMChannel):
            await ctx.send("En DM no se puede.")
            return False
    ...
    @commands.command(name="connect", aliases=["join", "unirse", "ven"])
    async def connect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):
        player = self.get_player(ctx)
        channel = await player.connect(ctx, channel)
        await ctx.send(f"Voy a  {channel.name}.")

    @connect_command.error
    async def connect_command_error(self, ctx, exc):
        if isinstance(exc, AlreadyConnectedToChannel):
            await ctx.send("Ando en otro lado.")
        elif isinstance(exc, NoVoiceChannel):
            await ctx.send("Canal no encontrado.")

    @commands.command(name="disconnect", aliases=["leave", "sacatela", "irse"])
    async def disconnect_command(self, ctx):
        player = self.get_player(ctx)
        await player.teardown()
        await ctx.send("Me la saco.")

    @commands.command(name="play", aliases = ["p", "ponte", "reproducir"])
    ...
    @commands.command(name="pause", aliases = ["pausa", "pausar"])
    ...
    async def pause_command_error(self, ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            await ctx.send("Ya en pausa.")

    @commands.command(name="stop", aliases=["para", "yamete"])
    ...
    @commands.command(name="next", aliases=["skip", "siguiente"])
    async def next_command(self, ctx):
        ...
        await player.stop()
        await ctx.send("Reproduciendo siguiente.")

    @next_command.error
    async def next_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("No nos pongamos filosóficos.")
        elif isinstance(exc, NoMoreTracks):
            await ctx.send("No hay siguiente.")
    ...
    async def previous_command(self, ctx):
        ...
        await player.stop()
        await ctx.send("Reproduciendo la anterior.")
    ...
    async def previous_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            ...
            await ctx.send("No hay anterior.")

    @commands.command(name="shuffle", aliases = ["barajar"])
    async def shuffle_command(self, ctx):
        player = self.get_player(ctx)
        player.queue.shuffle()
        await ctx.send("Cola barajada.")
    ...
    async def repeat_command(self, ctx, mode: str):
        ...
        await ctx.send(f"Repetición en modo: {mode}.")
    ...
    async def queue_command(self, ctx, show: t.Optional[int] = 10):
        ...
        embed = discord.Embed(
            title="Cola",
            description=f"Mostrando {show} pistas",
            colour=ctx.author.colour,
            timestamp=dt.datetime.utcnow()
        )
        embed.set_author(name="Resultados de cola")
        embed.set_footer(text=f"Solicitado por:{ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed.add_field(
            name="Reproduciendo:",
            value=getattr(player.queue.current_track, "title", "No hay pistas en reproducción."),
            inline=False
        )
    ...
    @commands.group(name="volume", invoke_without_command=True, aliases = ["volumen"])
    ...
    async def volume_group_error(self, ctx, exc):
        if isinstance(exc, VolumeTooLow):
            await ctx.send("No aprendí volúmenes negativos.")
        elif isinstance(exc, VolumeTooHigh):
            await ctx.send("Eso no es saludable 150% o menos.")

    @volume_group.command(name="up", aliases = ["subir", "súbele"])
    async def volume_up_command(self, ctx):
        ...
        await ctx.send(f"Volumen cambiado a {value:,}%")

    @volume_up_command.error
    async def volume_up_command_error(self, ctx, exc):
        if isinstance(exc, MaxVolume):
            await ctx.send("Volumen máximo.")

    @volume_group.command(name="down", aliases = ["bajar"])
    ...
    async def eq_command(self, ctx, preset: str):
        ...
        await ctx.send(f"Ecualizador ajustado {preset} .")

    @eq_command.error
    async def eq_command_error(self, ctx, exc):
        if isinstance(exc, InvalidEQPreset):
            await ctx.send("Las opciones son: 'flat', 'boost', 'metal', or 'piano'.")
    ...
    async def adveq_command(self, ctx, band: int, gain: float):
        ...
        await player.set_eq(eq)
        await ctx.send("Ecualizador ajustado.")
    ...
    async def playing_command(self, ctx):
        ...
        embed.set_author(name="Info de reproducción")
        embed.set_footer(text=f"Solicitado por: {ctx.author.display_name}", icon_url=ctx.author.avatar_url)
        embed.add_field(name="Título de la pista", value=player.queue.current_track.title, inline=False)
        embed.add_field(name="Artista", value=player.queue.current_track.author, inline=False)
    ...
    async def skipto_command(self, ctx, index: int):
        ...
        await player.stop()
        await ctx.send(f"Playing pista en posición {index}.")
    ...
    async def restart_command(self, ctx):
        ...
        await player.seek(0)
        await ctx.send("Pista Reiniciada.")
    ...
    async def seek_command(self, ctx, position: str):
        ...
        await ctx.send("Fui a buscar.")

    @commands.command(name = "addmusich", alisases = ["addmch"])
    async def setch_command(self, ctx, channel: discord.TextChannel):
      data = dbn.check("data_config/servers.json")
      data[str(message.guild.id)]["music"] = data[str(message.guild.id)].get("music", {"channels": []})
      data[str(message.guild.id)]["music"].channel.append(channel)
